<template>
    <div>
      ...
      <div v-if="movies.length" class="movie-list">
        ...
        >
          ...
          <button 
            :class="['buy-button', mainStore.cart.has(movie.id) ? 'added' : '']" 
            @click.stop="toggleCart(movie.id)"
          >
            {{ mainStore.cart.has(movie.id) ? 'Added' : 'Buy' }}
          </button>
        </div>
      </div>
  ...
  function toggleCart(movieId) {
    if (mainStore.cart.has(movieId)) {
      mainStore.removeFromCart(movieId);
    } else {
      mainStore.addToCart(movieId, movieId);  
    }
  }
